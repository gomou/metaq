/*
 * $Id: Socket.cpp 3 2011-08-19 02:25:45Z  $
 */
#include "Socket.h"
#include "Utility.h"
#include <assert.h>
...
namespace LWPR
{
	...
	LWPR::SOCKET_RET_TYPE_E Socket::ReadSocketAsPossible(SOCKET_FD_T fd, LWPR::Buffer& buf, int timeout)
	{
		SOCKET_RET_TYPE_E nRetResult = SOCKET_RET_FAILED;

		if(buf.RemainderSize() < 0) return SOCKET_RET_FAILED;

		nRetResult = IsSocketReadable(fd, timeout);
		if(nRetResult == SOCKET_RET_OK)
		{
			int nRetNum = read(fd, buf.Inout() + buf.Size(), buf.RemainderSize());
			...
			buf.Size(buf.Size() + nRetNum);
		}

		return nRetResult;
	}

	bool Socket::HostName2Value(const char* fromhost, std::string& tohost)
	{
	...
	const char* GetSocketInvokeRetDesc(SOCKET_INVOKE_RET_E ret)
	{
		switch(ret)
		{
		case SOCKET_INVOKE_OK:
			return "RPC: 调用成功";
		case SOCKET_INVOKE_CONNECT_FAILED:
			return "RPC: 网络连接失败";
		case SOCKET_INVOKE_SEND_DATA_FAILED:
			return "RPC: 发送数据失败";
		case SOCKET_INVOKE_SEND_DATA_TIMEOUT:
			return "RPC: 发送数据超时";
		case SOCKET_INVOKE_RECEIVE_DATA_FAILED:
			return "RPC: 接收数据失败";
		case SOCKET_INVOKE_RECEIVE_DATA_TIMEOUT:
			return "RPC: 接收数据超时";
		case SOCKET_INVOKE_POST_PROCESSING_FAILED:
			return "RPC: 后期数据处理失败";
		default:
			return "RPC: Unknow";
		}
