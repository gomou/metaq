/*
 * $Id: ThreadPool.cpp 16 2011-08-29 02:03:45Z  $
 */
...
namespace LWPR
{
	...
	void ThreadPool::Destroy()
	{
		...
		m_pThreadManage->Notify();
		m_pThreadManage->Join();
		m_pThreadManage->DecRef();
	}
	...
	void ThreadPool::ThreadManage::Run()
	{
		...
		// 清理线程资源
		for(int i = 0; i < 16 && m_ThreadPool.m_ThreadWorkerList.size() > 0; i++)
		{
			m_ThreadPool.ClearDeadThread();
			Thread::Sleep(1);
		}
